...
module.exports = async (req, res) => {
  ...
  console.log('Dynamic invite handler - Received request for path:', req.url);
  ...
  if (req.method !== 'GET') {
    ...
    // In Vercel, req.url might be /api/v2/invites-dynamic?code=ABCD1234 or similar if rewritten
    // But assuming the path is passed or we parse it from the URL
    // If this is a dynamic route like /api/v2/invites/[code], we might get it from req.query
    // Let's assume the URL structure is preserved or passed via query param if rewritten
    
    // If using Vercel rewrites, we might need to check req.query.inviteCode if configured
    // Or parse req.url. Let's try to be robust.
    ...
    if (inviteError || !invite) {
      ...
      return res.status(404).send(generateErrorPage('Invite Not Found', 'This invite code does not exist or has been removed.'));
    }

    // Check if invite is already redeemed
    if (invite.redeemed) {
      return res.status(410).send(generateErrorPage('Invite Already Used', 'This invite code has already been used by another user.'));
    }
    ...
    if (now > expiresAt) {
      return res.status(410).send(generateErrorPage('Invite Expired', 'This invite code has expired and is no longer valid.'));
    }
    ...
    return res.status(200).send(invitePage);
    
  } catch (error) {
    console.error('Dynamic invite handler error:', error);
    return res.status(500).send(generateErrorPage('Server Error', 'An error occurred while loading this invite.'));
  }
};
