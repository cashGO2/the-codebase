...
layout: post
image: "/assets/img/covers/a8881e71-8d77-4a5e-9772-4685c43255dc.webp"
---
...
See [http://oreilly.com/catalog/errata.csp?isbn=9781098156152](http://oreilly.com/catalog/errata.csp?isbn=9781098156152) for release details.
...
- 1. Introduction to Prompt Engineering. Part I. Foundations
   - LLMs Are Magic
...
- 4. Designing LLM Applications.
   ...
   - Evaluating LLM Application Quality
      ...
      - Online Evaluation
   - Conclusion
- 5. Prompt Content. Part II. Core Techniques
   - Sources of Content
   - Static Content
...
- 7. Taming the Model.
   ...
   - Choosing the Model
   - Conclusion
   - 8. Conversational Agency. Part III. An Expert of the Craft
      - Tool Usage
...
```
1.1. Tokens from the source language are sent to the encoder one at a time and
converted to an embedding vector, and they update the internal state of the
encoder.
2.2. The internal state is packaged up as the thought vector and sent to the decoder.
```
**6 | Chapter 1: Introduction to Prompt Engineering**


```
3.3. A special “start” token is sent to the decoder, indicating that this is the start of the
output tokens.
4.4. Conditioned upon the value of the thought vector, the decoder state is updated
and an output token from the target language is emitted.
5.5. The output token is provided as the next input into the decoder. At this point, the
process recurrently loops back and forth from step 4 to step 5.
6.6. Finally, the decoder emits a special “end” token, indicating that the decoding
...
Years’ War (listen on acast at [http://.](http://.) Of course you don’t know the URL
...
```
```
1.1. Each minibrain has some things it wants to know, so it submits a couple of
...
talking?”
2.2. Each minibrain has some things it can share, so it submits a couple of items, in
...
right now is Susan.”
3.3. Now, every question is matched up with its best-fitting answer. “Who is talking?”
matches up very well with “The person talking right now is Susan.”
4.4. The best-fitting answer to each question is revealed to the minibrain that asked
...
```
1.1. The prompt must closely resemble content from the training set.
2.2. The prompt must include all the information relevant to addressing the user’s
problem.
3.3. The prompt must lead the model to generate a completion that addresses the
problem.
4.4. The completion must have a reasonable end point so that generation comes to a
natural stop.
```
...
```
- _"North Korea fires ballistic missile, Japan says"_
- _"Five-day COVID-19 lockdown imposed in Pyongyang"_
- _"Yoon renews efforts to address dire North Korean human rights"_
...
```
```
1.1.What if you leave off ## Solution 2 or even the question that precedes it? Does
...
2.2.Problem 1 serves as an example. If you change the solution 1 text, does it modify
...
3.3.Try keeping the same country but replacing the negative context with increas‐
ingly positive remarks. Does the model still recommend against travel to North
Korea? Why might this be?
4.4.If you omit the stop word, then does the model confabulate a third problem? If
...
5.5.Are there any reasons that using a homework problem might be problematic?
...
```
1.1. Anything the application has directly at its fingertips, like anything to do with the
current state of the application (e.g., what’s currently written on the screen) or the
system (e.g., the current time and date)
2.2. What the application has saved somewhere (e.g., the user’s profile info)
3.3. Information the application could record for itself, even if it doesn’t yet (e.g.,
...
```
4.4. Information the application could obtain by using public APIs (e.g., the current
weather)
5.5. Information the application could obtain by asking the user directly or accessing
...
```
1.1. Things that are always the same for the same user (e.g., profile information)
2.2. Things that change slowly over time (e.g., purchase histories)
3.3. More ephemeral things (e.g., time, states of the user’s interaction with the app)
```
...
```
1.1. Split up the documents into smaller snippets.
2.2. Convert all of the snippets into embedding vectors, using the process described
above.
3.3. Insert the snippets and their corresponding vectors into a vector datastore of
your choice.
```
...
```
1.1. Make sure that the number of tokens is less than the maximum number of tokens
allowed for your embedding model. (As of 2024, the OpenAI embedding models
have a window of 8,191 tokens.)
2.2. Ideally, make sure that the text chunk is large enough to hold one and only one
...
3.3. Make sure the snippet is an appropriate size for placement in the prompt.
```
...
```
1.1. For chain-of-thought prompting or managing overly verbose models, you can
...
2.2. You can easily signal when the model’s response should end by adding a section
...
```
1.1. You could use what we call elastic prompt elements, which are prompt elements
...
2.2. Alternatively, you could create multiple prompt elements from the retrieved
...
```
1.1. Only allow your application to show corrections if it is confident.
2.2. Include warnings when the model struggles more than usual.
3.3. Incorporate more context or retry when the model struggles.
4.4. Switch to a more intelligent (and expensive) LLM for better results.
5.5. Only interrupt the user with assistance if the certainty that it is necessary is high.
...
```
1.1. Try to make your prompt look like the beginning of one of the documents you
fine-tuned for.
2.2. Be very sure not to have it look like one of the original documents instead.
```
```
...
```
1.1. Who should speak? The OpenAI API, rather than the model, inserts <|im_start|
...
2.2. Should a tool be called? The next tokens, to=functions., are generated by the
...
3.3. Which tool should be called? The next tokens the model generates represent the
name of the function: in this case, set_room_temp\n.
4.4. Which argument should be specified? The next text generated from the model
...
5.5. What value will the argument have? The model next predicts the value that
...
6.6. Are we done? Once all the arguments have been specified, the model predicts
...
```
1.1. Whatever is easier for a human to understand is also easier for an LLM to
understand.
```
...
```
2.2. The best results are derived by patterning prompts after training data (a.k.a. the
...
```
1.1. When the desired value is known in the application, remove the arguments from
...
tion.
2.2. Instruct the model to ask if it’s unsure about an argument—and then pray it does,
...
flights:
```
**_- 8/14/2023 9:20AM, flight no. JL5441 from IAD to SEA
- 8/14/2023 4:50PM, flight no. AS325 from IAD to SEA_**
_</artifact>_ "}
]
...
Example 8-2. The _run_conversation_ function manages the full conversation state,
...
Table 8-2. Executing _run_conversation_
...
```
1.1. Generate a list of popular Shopify storefronts and retrieve their website HTMLs.
2.2. For each storefront, extract details—product offering, branding, style, values, etc.
3.3. Review each storefront and come up with a plug-in that would benefit its busi‐
ness.
4.4. Generate marketing emails advertising the plug-in concept to each storefront
owner.
5.5. Send the emails.
```
...
```
1.1. Define goal. Identify the purpose of the workflow. What is the desired output or
desired change that the workflow will accomplish?
2.2. Specify tasks. Break the workflow down into a set of tasks that, when executed in
...
3.3. Implement tasks. Build the tasks as specified. Make sure input and output are
...
```
4.4. Implement workflow. Connect the tasks into a complete workflow. If necessary,
adjust tasks to ensure they function correctly in the context of the full workflow.
5.5. Optimize workflow. Optimize tasks to improve quality, performance, and cost.
```
...
1.1. Generate a list of popular Shopify storefronts and retrieve their website HTMLs.
2.2. For each storefront, extract details—product offering, branding, style, values, etc.
3.3. Review each storefront and come up with a plug-in that would benefit their
business.
4.4. Generate marketing emails advertising the plug-in concept to each storefront
owner.
5.5. Send the emails.
...
1.1. What do they sell?
2.2. What is the overall tone of the website? Fun? Serious? Relaxing?

**214 | Chapter 9: LLM Workflows**


```
3.3. What values do they hold most dear? Sustainability? Social causes?
4.4. What themes are present on the website? Travel? Productivity? Exercise?
5.5. Is there anything praiseworthy on the website? (We’re getting ready to stroke
their ego in the email!)
6.6. Is there anything else that seems noteworthy?
```
...
```
1.1. Where do you get the example problems?
2.2. How do you assess your app’s solutions to those problems?
```
...
```
1.1. Take an open source repository, and from it, take one code file, and from that file,
take one function.
2.2. Remove the body of that function, imagining that the user was just writing
...
would go.
3.3. Ask GitHub Copilot what to type next.
```
...
```
1.1. Direct feedback: what does the user say to the suggestion?
2.2. Functional correctness: does the suggestion work?
3.3. User acceptance: does the user follow the suggestion?
4.4. Achieved impact: how much does the user benefit?
5.5. Incidental metrics: what are the measurements “around” the suggestion?
```
...
```
1.1. LLMs are nothing more than text completion engines that mimic the text they
see during training.
2.2. You should empathize with the LLM and understand how it thinks.
```
