...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        if (event.httpMethod === 'GET') {
            ...
            const useAI = event.queryStringParameters?.useAI === 'true';
            const aiMode = event.queryStringParameters?.aiMode || 'hybrid'; // 'hybrid' or 'pure'
            const threshold = parseFloat(event.queryStringParameters?.threshold) || 0.4;
        ...
        if (event.httpMethod === 'POST') {
            const { query, useAI = false, aiMode = 'hybrid', threshold = 0.4 } = JSON.parse(event.body || '{}');
